...
export default function PozisyonTanimlama() {
    ...
    const submit = async (val) => {
        try {
            if (!val.id) await PostWithToken('CboPozisyon/Create', { ad: val.ad });
            else {
                const res = await PostWithToken('CboPozisyon/Update', { id: val.id, ad: val.ad });
    ...
    const deleteData = async (data) => {
        try {
            const res = await PostWithToken('CboPozisyon/Delete', { Id: data.id });
            if (res?.data?.isError) { AlertFunction('Hata', res.data.message); return; }
            setRefreshDatatable(new Date());
        } catch (e) { AlertFunction('Başarısız işlem', e?.response?.data?.message || 'Bir hata oluştu'); }
    };
    ...
    const editData = async (data) => {
        try {
            const res = await GetWithToken('CboPozisyon/GetById', { id: data.id });
            setInitialData(res?.data?.data || {});
            setModalOpen(true);
        } catch (e) { AlertFunction('Hata', 'Pozisyon yüklenemedi'); }
    };
                    ...
                    <Formik initialValues={formInitial} onSubmit={submit} enableReinitialize>
                        {({ handleSubmit }) => (
                            <Form onSubmit={handleSubmit} className="row">
                                ...
                                <div className="col-12 mb-3"><label className="form-label">Pozisyon Adı</label><Field name="ad" type="text" className="form-control" required /></div>
