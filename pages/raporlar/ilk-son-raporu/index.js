...
const basliklar = [
    ...
    { key: 'giris', label: 'Giris' },
    { key: 'cikis', label: 'Cikis' },
    { key: 'girisSaati', label: 'GirisSaati' },
    { key: 'cikisSaati', label: 'CikisSaati' },
    { key: 'toplamSure', label: 'ToplamSure' },
];

export default function IlkSonRaporuIndex() {
    ...
    const raporuGetir = async () => {
        ...
        setReportData(null);
        try {
            const res = await PostWithToken('GirisCikisRapor/GetIlkSonReport', {
                sicilIds: ids,
    ...
    const exportToExcel = async () => {
        ...
        const sheet = workbook.addWorksheet('İlk Son Raporu');
        ...
        reportData.forEach((row) => {
            sheet.addRow(basliklar.map((h) => {
                const val = row[h.key];
                if (h.key === 'tarih' || h.key === 'giris' || h.key === 'cikis') return val ? new Date(val).toISOString() : '';
                return val ?? '';
            }));
        });
        ...
        a.href = url;
        a.download = `ilk-son-raporu-${baslangic}-${bitis}.xlsx`;
        a.click();
    ...
    return (
        <Layout>
            <PageHeader
                title="İlk Son Raporu"
                map={[
                    { url: 'raporlar', name: 'Raporlar' },
                    { url: 'raporlar/ilk-son-raporu', name: 'İlk Son Raporu' },
                ]}
            />
            <div className="content p-4">
                <div className="card">
                    <div className="card-header">
                        <h5 className="mb-0">İlk Son Raporu</h5>
                    </div>
                                        ...
                                        <tbody>
                                            {reportData.map((row, idx) => (
                                                ...
                                                >
                                                    ...
                                                    <td>{row.giris ?? ''}</td>
                                                    <td>{row.cikis ?? ''}</td>
                                                    <td>{row.girisSaati ?? ''}</td>
                                                    <td>{row.cikisSaati ?? ''}</td>
                                                    <td>{row.toplamSure ?? ''}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
